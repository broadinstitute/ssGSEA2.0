...
ptmGSEA <- function (
  ...
  #    before using them to calculate scores.
    ...
    ##save(gs.mat, gene.names, file='test.Rdata')
    ##cat(dim(gs.mat))
    ## overlap
    ...
    ## filter
    keep.idx <- which(apply(gs.mat, 1, function(x) ifelse( sum(!is.na(x)) < min.overlap, F, T)))
    ...
    #################################################
    ## Loop over gene sets
    score.matrix <- matrix(NA, nrow=N.gs, ncol=Ns)
    pval.matrix <- matrix(NA, nrow=N.gs, ncol=Ns)

    tmp <- apply(gs.mat, 1, function(x){
        ##gene.set <- x[!is.na(x)]
        ...
        gene.overlap = x[!is.na(x)]

        print(paste("overlap=", length(gene.overlap)))
        if( length(gene.overlap) < min.overlap ) return(list(score=rep(NA, Ns), pval=rep(NA,Ns)))


        OPAM <- project.geneset (data.array = m, gene.names = gene.names, weight = weight, statistic = statistic, gene.set = gene.overlap, nperm = ifelse(output.score.type == "ES", 1, nperm), correl.type = correl.type)


        if(output.score.type == 'ES')
            res <- list(score=OPAM$ES.vector, pval=OPAM$p.val.vector)
        if(output.score.type == 'NES')
            res <- list(score=OPAM$NES.vector, pval=OPAM$p.val.vector)

        return( res  )
    })
    ##save(tmp,file='tmp.RData')

    pval.matrix <- matrix(unlist(lapply(tmp, function(x) x$pval)), nrow=N.gs, byrow=T)
    score.matrix <- matrix(unlist(lapply(tmp, function(x) x$score)), nrow=N.gs, byrow=T)
    ##View(pval.matrix)
...
project.geneset <- function (data.array,
    ...
    n.cols <- ncol(data.array)
    n.rows <- nrow(data.array)
    ...
    for (sample.index in 1:n.cols) {
        ...
        gene.list <- data.array[, sample.index]
        ##ordered.gene.list <- order(gene.list, decreasing=T)
        ##names( ordered.gene.list ) <- names(gene.list) <- gene.names


        ##gene.set2 <- match(gene.set, gene.names)

        ## calculate enrichment score
        GSEA.results <- gsea.score (gene.list, gene.set, gene.names, correl.type, weight, statistic)
        ...
        if (nperm == 0) {
            ...
            perm.mat <- matrix(0, nrow=n.rows, ncol=nperm)
            perm.mat <- apply(perm.mat, 2, function(x) sample(1:n.rows))

            ## apply gsea.score function on data columns
            ES.tmp = apply(perm.mat, 2, function(x) gsea.score(x, gene.set, gene.names, correl.type, weight, statistic)$ES)
            ...
            ##    GSEA.results = gsea.score (reshuffled.gene.labels, gene.set, gene.names, correl.type, weight, statistic)
...
##
###############################################################################
gsea.score <- function (gene.list, gene.set, gene.names, correl.type, weight, statistic = "area.under.RES") {

...
ptmGSEA <- function (
  ...
  #    before using them to calculate scores.
    ...
    if (sample.norm.type == "rank") {
        ...
        ## m <- apply(m, 2, rank, ties.method = "average")
        ##m <- apply(m, 2, function(x) rank(x[which(!is.na(x))], ties.method="average"))
        m <- apply(m, 2, function(x){
            x.rank=rep(NA, length(x))
            keep.idx=which(!is.na(x))
            x.rank[keep.idx ]=rank(x[keep.idx], ties.method="average")
...
project.geneset <- function (data.array,
                             ...
                             gene.set.direction=NULL,             ## direction of regulation; should be in same order than 'gene.set'
                             min.overlap
                             ) {
        ...
        for (sample.index in 1:n.cols) {
            ...
            if(length(gene.set) < min.overlap){
                ##ES.vector[sample.index] <- ES.vector[sample.index] <- p.val.vector[sample.index] <- NA
